  ...
  * \delta(p)=0 and \beta(p) > 0: Exponential growth, strictly increasing
  ...
  $$=-(\beta_1-\delta_1)(-\frac{\beta_0-\delta_0}{\beta_1-\delta_1}-p)p$$
	- ### Logistic Model Growth
	  ...
	  $$\text{or}$$
	  $$=\frac{P_0M}{0+P_0}=M$$
	  Which means that the long term behavior of this model is that the population approaches the carrying capacity (*M*), which makes perfect logistical sense. Obviously, this breaks, as does the model, if P_0 >